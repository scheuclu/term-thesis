...
def parse(path):
    ...
    re_tikzFile=re.compile(r'\\tikzFile(?:\[.*?\])?{(.*?)}{(.+?)}', re.MULTILINE)
    ...
    filedir = ''.join(path.split(os.sep)[:-1])
    # Files that are included via \input or \include or \tikzFile
    for tex in re_inc.findall(data)+re_inp.findall(data):
        tex =    os.path.join(filedir,tex)
    ...
    # Files that are included via \tikzFile
    #for tikzFile in re_tikzFile.findall(data):
    #    fullFile=os.path.join(filedir,tikzFile[0], tikzFile[1]) 
    #    imported_tikz.add(fullFile+'.tikz')
...
#            raise Exception
    ...
    for impFile in re_imp.findall(data):
        impFile=os.path.join(filedir,impFile[0], impFile[1])

        if not(impFile.endswith('.pdf_tex') or impFile.endswith('.tikz')):
            red('\nFile extension of imported files must be ".pdf_tex" or ".tikz".')
            red('File: '+impFile)
            red('Imported in file: '+path+'\n')
            raise Exception

        if os.path.exists(impFile[0:-8]+'.svg'):
            imported_svg.add(impFile[0:-8]+'.svg')
        else:
            red('\nFile not found: '+impFile[0:-8]+'.svg')
            red('Needed by file: '+path+'\n')
            raise Exception
...
#                 raise Exception
    ...
    return imported_svg, included_svg, imported_tikz, included_pdf, included_sub


class CleanFile:
    tex_clean=['.aux', '.log', '.idx', '.toc', '.out']
