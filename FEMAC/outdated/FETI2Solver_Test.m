classdef FETI2Solver_Test < FETIBaseSolver_Test
...
%   Detailed explanation goes here
  ...
  methods
    ...
    %% ctor FETI2Solver
    function this = FETI2Solver_Test(dis,outbuffer,params,condlist)% {{{ctor
    ...
    function [] = Setup(this)
      ...
      for s = 1:this.fetiman_.gNumSub()
        if this.params_.ASSEMBLEDOPS
          this.St_ = this.St_ + this.fetiman_.Bs_{s}(:,this.fetiman_.ifacedofslids_{s}) *...
                     this.fetiman_.Ks_{s}(this.fetiman_.ifacedofslids_{s},this.fetiman_.ifacedofslids_{s}) *...
                     this.fetiman_.Bs_{s}(:,this.fetiman_.ifacedofslids_{s})';
        end
      end
      ...
      if this.params_.ASSEMBLEDOPS
        ...
        temp = B*B';
        temp = pinv(full(temp));
        %temp=temp\eye(full(size(temp)));%TODO aks whether this would solve the pinv problem for sparse systems
    ...
    function [status] = Solve(this)
      ...
      while (true)
        ...
        r(:,k+1) = r(:,k) - (gamma/delta).*this.P_'*(q(:,k));              %r(:,k) - (gamma/delta).*this.APPL_PNT(q(:,k));
